...
combineTwoAudio() {
  ...
  merge_audio_command="ffmpeg -i $audio_file_path -i $audio_extract_file_path -filter_complex amerge -c:a libmp3lame -q:a 4 $merge_file_path";
...
divideAudio() {
  SAMPLING_DURATION=5;
  merge_file_path="$output_path/1588245746_audio_merged.mp3";
